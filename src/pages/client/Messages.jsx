...
const ClientMessages = () => {
  ...
  const fetchMissions = async () => {
    ...
    const { data } = await supabase
      ...
      .select(`
        *,
        prestataire:profiles!missions_prestataire_id_fkey(id, nom, avatar_url)
      `)
  ...
  return (
    <div className="h-screen flex flex-col" style={font}>
      <Navbar />
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className={`${showChat ? 'hidden md:flex' : 'flex'} w-full md:w-80 flex-shrink-0 bg-white border-r border-gray-100 flex-col`}>
          ...
          {loading ? (
            ...
            <div className="flex-1 flex items-center justify-center p-6 text-center">
              <p className="text-gray-400 text-xs font-light">Aucune conversation pour l instant</p>
            </div>
          ) : (
              ...
              {missions.map(mission => (
                <button
                  ...
                  className={`w-full px-4 py-4 text-left border-b border-gray-50 transition-all ${
                    selectedMission?.id === mission.id ? 'bg-gray-50 border-l-2 border-l-black' : 'hover:bg-gray-50'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <Avatar url={mission.prestataire?.avatar_url} nom={mission.prestataire?.nom} size="md" />
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-gray-900 truncate">{mission.prestataire?.nom}</p>
                      <p className="text-xs text-gray-400 font-light truncate mt-0.5">{mission.titre}</p>
                    </div>
                  </div>
                </button>
              ))}
        ...
        <div className={`${!showChat ? 'hidden md:flex' : 'flex'} flex-1 flex-col bg-white`}>
          {!selectedMission ? (
            <div className="flex-1 flex items-center justify-center">
              <p className="text-gray-400 text-sm font-light">Selectionnez une conversation</p>
            </div>
          ) : (
            <>
              {/* Header chat */}
              <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-3 flex-shrink-0">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-gray-900 truncate">{selectedMission.prestataire?.nom}</p>
                  <p className="text-xs text-gray-400 font-light truncate">{selectedMission.titre}</p>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {messages.length === 0 ? (
                  <div className="flex items-center justify-center h-full">
                    <p className="text-gray-400 text-xs font-light">Aucun message pour l instant. Demarrez la conversation !</p>
                  </div>
                ) : (
                  messages.map((msg, i) => {
                      ...
                      <div key={msg.id}>
                        ...
                        <div className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                          <div className="max-w-xs sm:max-w-sm md:max-w-md">
                            {!isMe ? (
                              <p className="text-xs text-gray-400 font-light mb-1 px-1">{msg.expediteur?.nom}</p>
                            ) : null}
                            ...
                            <p className={`text-xs text-gray-400 font-light mt-1 px-1 ${isMe ? 'text-right' : 'text-left'}`}>
