    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Historique des missions</h1>
          ...
          <div className="relative">
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
              ...
              className="pl-9 pr-4 py-2.5 border border-gray-200 bg-white rounded-xl text-sm text-gray-900 placeholder-gray-300 focus:outline-none focus:border-gray-900 focus:ring-2 focus:ring-gray-900/10 transition-all shadow-card w-56" />
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <Link to="/prestataire/missions" className="text-xs text-gray-400 hover:text-gray-900 underline">
          ...
          <div className="flex gap-6">
            ...
            <div className="w-72 flex-shrink-0 space-y-2">
              {missionsFiltrees.length === 0 ? (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-6 text-center">
                  <p className="text-gray-400 text-xs">Aucune mission dans cette categorie</p>
                </div>
              ) : (
                missionsFiltrees.map(mission => (
                  <button key={mission.id} onClick={() => setSelected(mission)}
                    ...
                    <div className="flex items-center justify-between mb-1">
                      <span className={`text-xs ${selected?.id === mission.id ? 'text-gray-300' : 'text-gray-400'}`}>
                        {mission.client?.nom}
                      </span>
                      <StatusBadge statut={mission.statut} />
                    </div>
                    <span className={`text-xs font-bold ${selected?.id === mission.id ? 'text-white' : 'text-gray-900'}`}>
            ...
            {selected ? (
              <div className="flex-1 space-y-4">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-6">
                  ...
                  {selected.statut === 'valide' ? (
                    <div className="flex items-center gap-3 p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
                      ...
                      <div>
                        <p className="text-sm font-bold text-emerald-800">Mission terminee avec succes</p>
                        <p className="text-xs text-emerald-600 mt-0.5">Vous avez gagne {selected.budget?.toLocaleString()} FCFA</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 p-4 bg-red-50 border border-red-200 rounded-xl">
                      ...
                      <div>
                        <p className="text-sm font-bold text-red-800">Mission contestee</p>
                        <p className="text-xs text-red-600 mt-0.5">Le client a conteste la livraison</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between p-4 bg-blue-50 border border-blue-200 rounded-xl">
                      <div>
                        ...
                        <p className="text-xs text-blue-600 mt-0.5">Marquez la mission comme livree quand vous avez termine</p>
                      </div>
                      <button onClick={() => handleLivrer(selected.id)}
                        className="px-4 py-2 bg-gray-900 text-white text-xs font-semibold rounded-xl hover:bg-black transition-all flex-shrink-0 ml-4">
                        Marquer livre
                      </button>
                    </div>
                  ) : null}
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-6">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-semibold text-gray-900">{selected.client.nom}</p>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-6">
                  <h3 className="font-bold text-gray-900 text-sm mb-4">Avis recu</h3>
                  {avisSelected.length === 0 ? (
                    <p className="text-xs text-gray-400">
                      {selected.statut === 'valide'
                        ? 'Le client n a pas encore laisse d avis'
                        : 'Aucun avis pour cette mission'}
                    </p>
                  ) : (
                      ...
                      {avisSelected.map((a, i) => (
                        <div key={i} className="p-4 bg-gray-50 rounded-xl">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <div>
                              <p className="text-xs font-semibold text-gray-900">{a.auteur?.nom}</p>
